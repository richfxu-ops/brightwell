...
Agents execute well and decide dangerously; every decision must surface where the owner can judge it — by reasoning and reversibility, not implementation.

- **Name decisions as decisions.** Anything with a reasonable alternative — a dependency, a data shape, scope, where code lives — gets named with the alternative and the tradeoff. Reporting what was *done* without what was *decided* is incomplete; "while I was at it" scope expansion is a decision too.
- **Sort by reversibility.** Two-way doors (cheap to undo: naming, layout, internal refactors) — decide, proceed, note the choice. One-way doors (dependencies, stored formats, public APIs/schemas, anything other code builds on) — propose first: options, tradeoff, recommendation, cost of being wrong; then wait for sign-off. Unsure which? Treat as one-way.
- **Meet the explanation bar.** A decision the owner can't understand well enough to *disagree with* isn't ready for approval — reframe until they could argue the other side. Never present a choice as a non-choice ("the standard approach" must name what it beat).
...
**Keep `INDEX.md` current.** Any structural change to the repo belongs in `INDEX.md` in the same change — adding an important file, module, or doc (engine module, content file, script, reference doc); moving, renaming, or removing one; or reorganizing the layout. Add, update, or delete the affected pointer(s) so the map never lies — a stale index is worse than none.
